...
import com.sad25kag.BuildConfig
...
class IStreamFlare : MainAPI() {
            ...
            seasons.forEach { season ->
                ...
                if (
                    ...
                    episodes.forEach { episode ->
                        ...
                        val epNumber =
                            episode.episoade_order
                                .toIntOrNull()
                                ?: return@forEach

                        // Use the play-links endpoint as episode data
                        // App flow (Frida): tapping Play → getMoviePlayLinks/{id}/0
                        val episodeData = if (episode.url.contains(".php?id=", ignoreCase = true)
                            || episode.url.startsWith("http")
                        ) {
                            // raw stream or CDN extractor URL — pass directly
                            episode.url
                        } else {
                            // episode id → build the play-links endpoint
                            "$mainUrl/android/getMoviePlayLinks/${episode.id}/0"
                        }

                        if (episodeData.isNotBlank()) {
    ...
    override suspend fun loadLinks(
        ...
        suspend fun processLinks(links: List<StreamLinks>) {
            links.forEach { link ->
                if (link.url.isBlank()) return@forEach
                when {
                    // x7flix URLs also contain .php?id= in their path
                    link.url.contains("x7flix", true) -> {
                        callback.invoke(
                            newExtractorLink(
                                name = link.name.ifBlank { name },
                                source = this.name,
                                url = link.url,
                    ...
                    // CDN .php?id= redirect — resolved via HttpURLConnection + Cyrillic UA
                    link.url.contains(".php?id=", true) -> {
                        loadExtractor(
                            link.url,
                            link.quality,
                            subtitleCallback,
                    ...
                    // Any other direct URL — infer type from extension
                    else -> {
        ...
        return when {
            ...
            // ── Episode play links endpoint ────────────────────────────────
            data.contains("getEpisodePlayLinks", ignoreCase = true) -> {
                val decodedJson = getDecodedJson(data)
                val links: List<StreamLinks> = try {
                    parseJson(decodedJson)
                } catch (_: Exception) { emptyList() }
            ...
            // x7flix URLs contain .php?id= in their path too
            data.contains("x7flix", true) -> {
                callback.invoke(
                    newExtractorLink(
                        name = name,
                        source = this.name,
            ...
            // ── .php?id= CDN link — resolved by Istreamcdn extractor ──────────
            data.contains(".php?id=", true) -> {
                loadExtractor(data, subtitleCallback, callback)
                true
            }

            // ── Live TV / direct stream URL ─────────────────────────────────
