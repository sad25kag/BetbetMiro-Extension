...
open class DonghuaFilmDailyMotion : ExtractorApi() {
    ...
    private fun extractId(url: String): String? {
        ...
        Regex("""[?&]video=([A-Za-z0-9]+)""").find(url)?.groupValues?.getOrNull(1)?.let { return it }
        Regex("""dai\.ly/([A-Za-z0-9]+)""").find(url)?.groupValues?.getOrNull(1)?.let { return it }
        return runCatching {
...
class DonghuaFilmOkRuSSL : DonghuaFilmOdnoklassniki() {
    override var name = "OkRuSSL"
    override var mainUrl = "https://ok.ru"
}

class DonghuaFilmOkRuHTTP : DonghuaFilmOdnoklassniki() {
    override var name = "OkRuHTTP"
    override var mainUrl = "http://ok.ru"
}

open class DonghuaFilmOdnoklassniki : ExtractorApi() {
